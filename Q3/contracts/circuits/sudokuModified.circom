// [assignment] please copy the entire modified sudoku.circom 
pragma circom 2.0.3;
...
template sudoku() {
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            ranges[i][j] = RangeProof(32);      // create a RangeProof component for every puzzle item
            ranges[i][j].range[0] <== 0;        // initialize range proof low bound
            ranges[i][j].range[1] <== 9;        // initialize range proof high bound
            ranges[i][j].in <== puzzle[i][j];   // initialize range proof's input signal as current puzzle element
            ranges[i][j].out === 1;             // add constraint that current puzzle element is within the range
